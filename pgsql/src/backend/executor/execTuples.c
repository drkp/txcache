 ...
 */
TupleTableSlot *
ExecStoreVirtualTuple(TupleTableSlot *slot, ValidityInterval valint)
{
	/*
	 * sanity checks
	 */
...
ExecStoreAllNullTuple(TupleTableSlot *slot)
{
	...
	return ExecStoreVirtualTuple(slot, InfiniteValidityInterval);
}
...
ExecCopySlotMinimalTuple(TupleTableSlot *slot)
{
	...
	if (slot->tts_tuple)
	{
		...
		{
			tup = minimal_tuple_from_heap_tuple(slot->tts_tuple);
			tup->t_valint = GetTupleValidity(slot);
			return tup;
		}
	}
...
ExecCopySlot(TupleTableSlot *dstslot, TupleTableSlot *srcslot)
{
	...
	dstslot->tts_valint = srcslot->tts_valint;
...
ValidityInterval
GetHeapTupleValidity(HeapTuple tuple)
{
	ValidityInterval r;
	HeapTupleHeader	header = tuple->t_data;
	XStamp xstamp;
	
	xstamp = XidToXStamp(HeapTupleHeaderGetXmin(header));
	/*
	 * xmin -> InvalidXStamp could indicate that xmin hasn't
	 * committed (but then we shouldn't be asking for the tuple's
	 * validity), or that xmin is older than all of our pinned
	 * snapshots and we removed it from the xid->xstamp map.
	 * So we return -infinity.
	 */
	r.start = XSTAMP_IS_VALID(xstamp) ? xstamp : XStampMinusInfinity;

	if ((header->t_infomask & HEAP_XMAX_INVALID) ||
		(header->t_infomask & HEAP_IS_LOCKED))
	{
		r.end = XStampInfinity;
	}
	else
	{
		/*
		 * if invalid, this probably means an incomplete/aborted
		 * transaction. Could also mean something that committed
		 * so early we don't know the xstamp, but then we
		 * wouldn't be seeing the tuple
		 */
		xstamp = XidToXStamp(HeapTupleHeaderGetXmax(header));
		r.end = XSTAMP_IS_VALID(xstamp) ? xstamp : XStampInfinity;
	}

#ifdef DEBUG_REPORT_XSTAMPS
	ereport(WARNING,
			(errcode(ERRCODE_WARNING),
			 (errmsg_internal("getting heap tuple validity: "
							  "xmin %ld xmax %ld flags %lx; "
							  "result is %ld %ld",
							  HeapTupleHeaderGetXmin(header),
							  HeapTupleHeaderGetXmax(header),
							  header->t_infomask,
							  r.start, r.end))));
#endif
	return r;	
...
ValidityInterval
GetTupleValidity(TupleTableSlot *slot)
{
	HeapTupleHeader header;

	if (slot->tts_isempty)
	{
		return InfiniteValidityInterval;
	}
	else if (slot->tts_mintuple != NULL)
	{
		return slot->tts_mintuple->t_valint;
	}
	else if (slot->tts_tuple != NULL)
	{
		/* physical tuple */
		header = slot->tts_tuple->t_data;

		/* we shouldn't be looking at tuples with invalid xmin... */
		Assert(!(header->t_infomask & HEAP_XMIN_INVALID));
		return GetHeapTupleValidity(slot->tts_tuple);
	}
	else
	{
		return slot->tts_valint;
	}
}
