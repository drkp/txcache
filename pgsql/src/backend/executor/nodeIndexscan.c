...
static TupleTableSlot *IndexNext(IndexScanState *node, bool timeQual);
...
IndexNext(IndexScanState *node, bool timeQual)
{
	EState	   *estate;
	...
	if ((tuple = index_getnext(scandesc, direction,
							   estate->es_xstamp, &inv)) != NULL)
	{
		estate->es_invalidityMask =
...
ExecIndexScan(IndexScanState *node)
{
	 ...
	 */
	return ExecScan(&node->ss, (ExecScanAccessMtd) IndexNext, false);
}
...
ExecIndexReScan(IndexScanState *node, ExprContext *exprCtxt)
{
	...
	if ((node->iss_NumScanKeys ==
		 node->iss_RelationDesc->rd_att->natts))
		AddInvalidationTag(node->ss.ss_currentRelation,
...
#define INDEXSCAN_NSLOTS 2
	...
	/*
	 * Create invalidation tags now if we have no runtime keys. If we
	 * have runtime keys, we have to wait until after we evaluate
	 * them.
	 */
	if (indexstate->iss_NumRuntimeKeys == 0)
	{
		if ((indexstate->iss_NumScanKeys ==
			 indexstate->iss_RelationDesc->rd_att->natts))
			AddInvalidationTag(currentRelation,
							   indexstate->iss_RelationDesc,
							   indexstate->iss_NumScanKeys,
							   indexstate->iss_ScanKeys);
		else 
