...
InternalCreateExecutorState(MemoryContext qcontext, bool is_subquery)
{
	...
	estate->es_xstamp = InvalidXStamp;
					  ...
					  bool invalidateOnly)
{
	...
	for (i = 0; i < numIndices; i++)
	{
		 ...
		 */
		InvalidateTag(heapRelation, relationDescs[i],
					  indexInfo, values, isnull);
	...
	/*
	 * If no indexes, send a wildcard invalidation for the table
	 */
	if (numIndices == 0)
		InvalidateTag(heapRelation, NULL, NULL, NULL, NULL);
}
