...
static TupleTableSlot *ValuesNext(ValuesScanState *node, bool timeQual);
...
ValuesNext(ValuesScanState *node, bool timeQual)
{
	TupleTableSlot *slot;
	EState	   *estate;
	...
	if (exprlist)
	{
		 ...
		 */
		ExecStoreVirtualTuple(slot, InfiniteValidityInterval);
	}
...
ExecValuesScan(ValuesScanState *node)
{
	 ...
	 */
	return ExecScan(&node->ss, (ExecScanAccessMtd) ValuesNext, false);
}
