		 ...
		 bool lateTimeQual)
{
	...
	/* Execute normally if the executor has no query xstamp */
	...
	for (;;)
	{
		...
		slot = (*accessMtd) (node, !lateTimeQual);
		...
		{
			/* Check time qual if we're doing it late */
			if (lateTimeQual)
			{
				LockBuffer(slot->tts_buffer, BUFFER_LOCK_SHARE);
				if (!HeapTupleSatisfiesVisibility(slot->tts_tuple,
												  estate->es_snapshot,
												  slot->tts_buffer))
				{
					LockBuffer(slot->tts_buffer, BUFFER_LOCK_UNLOCK);
					estate->es_invalidityMask =
						ValidityIntervalExcludeInvisibleTuple(estate->es_invalidityMask,
															  slot->tts_tuple,
															  estate->es_xstamp);
					continue;
				}
