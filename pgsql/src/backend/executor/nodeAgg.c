 ...
 *
 * IDENTIFICATION
 *	  $PostgreSQL: pgsql/src/backend/executor/nodeAgg.c,v 1.146.2.3 2008/10/16 19:26:01 neilc Exp $
 *
 *-------------------------------------------------------------------------
 */
	...
	while (!aggstate->agg_done)
	{
		...
		if (aggstate->grp_firstTuple == NULL)
		{
			...
			if (!TupIsNull(outerslot))
			{
				...
				aggstate->valint = ValidityIntervalIntersect(aggstate->valint,
															 GetTupleValidity(outerslot));
			}
			else
			{
		...
		aggstate->ss.ps.state->es_invalidityMask =
			ValidityIntervalIntersect(aggstate->ss.ps.state->es_invalidityMask,
									  aggstate->valint);
		...
		if (ExecQual(aggstate->ss.ps.qual, econtext, false))
		{
			 ...
			 */
			ExecProject(projInfo, NULL);
			projInfo->pi_slot->tts_valint =
				ValidityIntervalIntersect(projInfo->pi_slot->tts_valint,
										  aggstate->valint);
...
ExecInitAgg(Agg *node, EState *estate, int eflags)
{
	...
	aggstate->valint = InfiniteValidityInterval;
