...
{
	TupleTableSlot *slot;
	ValidityInterval valint = InfiniteValidityInterval;
	...
	if (projInfo->pi_exprContext->ecxt_scantuple != NULL)
	{
		valint = ValidityIntervalIntersect(valint,
										   GetTupleValidity(projInfo->pi_exprContext->ecxt_scantuple));
	}
	if (projInfo->pi_exprContext->ecxt_outertuple != NULL)
	{
		valint = ValidityIntervalIntersect(valint,
										   GetTupleValidity(projInfo->pi_exprContext->ecxt_outertuple));
	}
	if (projInfo->pi_exprContext->ecxt_innertuple != NULL)
	{
		valint = ValidityIntervalIntersect(valint,
										   GetTupleValidity(projInfo->pi_exprContext->ecxt_innertuple));
	}
	...
	if (projInfo->pi_isVarList)
	{
		...
		ExecVariableList(projInfo,
						 slot->tts_values,
						 slot->tts_isnull);
		ExecStoreVirtualTuple(slot, valint);
	}
	else
	{
