...
%type <node>	stmt schema_stmt
		...
		GrantStmt GrantRoleStmt IndexStmt InsertStmt InvalidateStmt
        ListenStmt LoadStmt
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	INTERVAL INTO INVALIDATE INVOKER IS ISNULL
    ISOLATION
	...
	PARTIAL PASSWORD PIN PLACING POSITION
	...
	SHOW SIMILAR SIMPLE SMALLINT SNAPSHOTID SOME STABLE START STATEMENT
	...
	UNCOMMITTED UNENCRYPTED UNION UNIQUE UNKNOWN UNLISTEN UNPIN UNTIL
	UPDATE USER USING
...
InvalidateStmt: INVALIDATE name
				{
					InvalidateStmt *n = makeNode(InvalidateStmt);
					n->tag = $2;
					$$ = (Node *)n;
				}
		;
...
TransactionStmt:
			...
			| PIN opt_transaction
				{
					TransactionStmt *n = makeNode(TransactionStmt);
					n->kind = TRANS_STMT_PIN;
					n->options = NIL;
					$$ = (Node *)n;
				}
			| UNPIN opt_transaction IntegerOnly
				{
					TransactionStmt *n = makeNode(TransactionStmt);
					n->kind = TRANS_STMT_UNPIN;
					n->options = list_make1(makeDefElem("snapshotid", $3));
					$$ = (Node *)n;
				}
...
transaction_mode_item:
			...
			| READ WRITE
					{ $$ = makeDefElem("transaction_read_only",
									   makeIntConst(FALSE)); }
			| SNAPSHOTID IntegerOnly
					{ $$ = makeDefElem("snapshotid", $2); }
		;
