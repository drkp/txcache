			   ...
			   char *completionTag)
{
	...
	switch (nodeTag(parsetree))
	{
						...
						foreach(lc, stmt->options)
						{
							...
							if (strcmp(item->defname, "transaction_isolation") == 0)
								...
								SetPGVariable("transaction_read_only",
											  list_make1(item->arg),
											  true);
							else if (strcmp(item->defname, "snapshotid") == 0) {
								XStamp ssnap =
									((Value *)item->arg)->val.ival + 1;
								SetTransactionXStamp(ssnap);
							}
						}
					}
					...
					case TRANS_STMT_PIN:
						{
							...
							ssnap = AssignAndPinTransactionSnapshot(&tp);
							snap = GetTransactionSnapshot();
							if (ssnap > FirstXStamp)
								SendNullInvalidation(ssnap-1);
							
							if (completionTag) {
								sprintf(completionTag,
										"PIN %lld %lld %lld",
										(long long)ssnap-1, (long long)tp.tv_sec, (long long)tp.tv_usec);
							}
						}
						break;

					case TRANS_STMT_UNPIN:
						{
							ListCell   *lc = list_head(stmt->options);
							DefElem    *item = (DefElem *) lfirst(lc);
							XStamp ssnap =
								((Value *)item->arg)->val.ival + 1;

							if (!RemoveSavedSnapshot(ssnap)) {
									ereport(ERROR,
											(errcode(ERRCODE_INVALID_NAME),
											 errmsg("saved snapshot not found: %d", ssnap)));
							}
							if (completionTag) {
								sprintf(completionTag,
										"UNPIN %ld",
		...
		case T_InvalidateStmt:
			{
				InvalidateStmt *stmt = (InvalidateStmt *) parsetree;

				if (!IsXactIsoLevelSerializable)
				{
					ereport(ERROR,
							(errcode(ERRCODE_INVALID_TRANSACTION_STATE),
							 errmsg("cannot INVALIDATE in a non-serializable transaction")));
				}
				EnqueueInvalidate(stmt->tag);
			}
			break;
