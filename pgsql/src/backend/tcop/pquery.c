				...
				DestReceiver *dest)
{
	...
	uint32		nprocessed;
	char completionTagBuf[COMPLETION_TAG_BUFSIZE];
	...
	if (forward)
	{
		...
		else
		{
			...
			if (XSTAMP_IS_VALID(queryDesc->estate->es_xstamp))
			{
				snprintf(completionTagBuf, COMPLETION_TAG_BUFSIZE,
						 "SELECT VALIDITY %ld %ld%s",
						 queryDesc->estate->es_finalValidity.start,
						 queryDesc->estate->es_finalValidity.end,
						 (queryDesc->estate->es_validityIsUnbounded ?
						  GetQueryTagSet() : ""));
				portal->commandTag = pstrdup(completionTagBuf);				
			}
