...
bool		XactReadOnly;
bool        XactWrote; 			/* indicates whether the transaction
								 * actually wrote, i.e. whether it
								 * would have been OK in R/O mode */
bool        XactInvalidated;
...
typedef struct TransactionStateData
{
	...
	bool		prevXactWrote;	/* entry-time xact r/o state */
	XStamp      xstamp;			/* xstamp, if using snapshotid */
	XStamp      latestXstampAtStart;
...
static TransactionStateData TopTransactionStateData = {
	...
	InvalidXStamp,				/* xstamp, if using snapshotid */
	InvalidXStamp,
...
StartTransaction(void)
{
	...
	s->xstamp = InvalidXStamp;
	s->latestXstampAtStart = InvalidXStamp;
...
CommitTransaction(void)
{
	...
	if (MyProc != NULL)
	{
		...
		if (XactWrote)
			GetCommitXStamp(xid, true); /* also sends invalidations */
		else if (XactInvalidated)
		{
			ereport(WARNING,
					(errcode(ERRCODE_INVALID_TRANSACTION_STATE), /* XXX */
					 errmsg("discarding invalidations because transaction "
							"was read-only")));
...
CommitSubTransaction(void)
{
	...
	XactWrote |= s->prevXactWrote;

	s->parent->latestXstampAtStart = s->latestXstampAtStart;
...
PushTransaction(void)
{
	...
	s->prevXactWrote = XactWrote;
	s->xstamp = p->xstamp;
	s->latestXstampAtStart = p->latestXstampAtStart;
...
XStamp
AssignAndPinTransactionSnapshot(struct timeval *tp)
{
	XStamp xstamp = SaveSnapshot(tp);
	CurrentTransactionState->xstamp = xstamp;
	return xstamp;
}

void
SetTransactionXStamp(XStamp xstamp)
{
	Snapshot s = GetSavedSnapshot(xstamp);
	if (s == NULL) {
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_NAME),
				 errmsg("saved snapshot not found")));
	} else {
		CurrentTransactionState->xstamp = xstamp;
		SetSerializableSnapshot(s);
		pfree(s);
	}
}

XStamp
GetTransactionXStamp()
{
	return CurrentTransactionState->xstamp;
}

XStamp
GetLatestXStampAtTransactionStart()
{
	return CurrentTransactionState->latestXstampAtStart;
}

void
SetLatestXStampAtTransactionStartIfUnset(XStamp xstamp)
{
	if (!XSTAMP_IS_VALID(CurrentTransactionState->latestXstampAtStart))
	{
		CurrentTransactionState->latestXstampAtStart = xstamp;
	}
}
