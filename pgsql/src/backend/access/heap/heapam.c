 ...
 */
static void
heapgetpage(HeapScanDesc scan, BlockNumber page, bool ignoreVis)
{
	Buffer		buffer;
	Snapshot	snapshot;
	Page		dp;
		 ...
		 lineoff++, lpp++)
	{
		if (ItemIdIsUsed(lpp))
		{
			...
			valid = ignoreVis ? true : HeapTupleSatisfiesVisibility(&loctup, snapshot, buffer);
			if (valid)
...
heapgettup(HeapScanDesc scan,
		   ...
		   ScanKey key,
		   XStamp now,
		   ValidityInterval *invalidityMask,
		   bool timeQual)
{
		...
		while (linesleft > 0)
		{
			if (ItemIdIsUsed(lpp))
			{
				...
				if (valid)
				{
					...
					{
						inv =
							ValidityIntervalExcludeInvisibleTuple(inv,
																  tuple,
																  now);
					}
				}
					...
					bool timeQual)
{
	...
	ValidityInterval inv = InfiniteValidityInterval;
	Snapshot	snapshot = scan->rs_snapshot;
	bool makeInvalidityMask = ((now != InvalidXStamp)
							   && (invalidityMask != NULL));
	bool		valid;
	...
	if (ScanDirectionIsForward(dir))
	{
		if (!scan->rs_inited)
		{
			...
			page = 0;			/* first page */
			heapgetpage(scan, page, makeInvalidityMask);
			lineindex = 0;
			scan->rs_inited = true;
		}
		else
...
HeapTuple
heap_getnext_im(HeapScanDesc scan, ScanDirection direction,
...
{
	return heap_getnext_im(scan, direction, InvalidXStamp, NULL, true);
}
