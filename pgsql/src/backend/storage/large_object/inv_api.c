...
inv_getsize(LargeObjectDesc *obj_desc)
{
	 ...
	 */
	while ((tuple = index_getnext(sd, BackwardScanDirection,
								  InvalidXStamp, NULL)) != NULL)
	{
...
inv_read(LargeObjectDesc *obj_desc, char *buf, int nbytes)
{
	...
	while ((tuple = index_getnext(sd, ForwardScanDirection,
								  InvalidXStamp, NULL)) != NULL)
	{
		Form_pg_largeobject data;
		bytea	   *datafield;
...
inv_write(LargeObjectDesc *obj_desc, const char *buf, int nbytes)
{
	...
	while (nwritten < nbytes)
	{
		...
		if (neednextpage)
		{
			if ((oldtuple = index_getnext(sd, ForwardScanDirection,
										  InvalidXStamp, NULL)) != NULL)
			{
