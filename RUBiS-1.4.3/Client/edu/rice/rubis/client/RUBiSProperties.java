...
public class RUBiSProperties
{
  ...
  private float   downSlowdown;
    // Austin
    public int upXactions, sessionXactions, downXactions;
  ...
  public URLGenerator checkPropertiesFileAndGetURLGenerator()
  {
    ...
    try
    {
      ...
      System.out.print("Server names      : ");
      StringTokenizer hostnames = new StringTokenizer(getProperty("httpd_hostnames"), ",");
      webSites = new Vector<URL>(hostnames.countTokens());
      while (hostnames.hasMoreTokens()) {
          String hostname = hostnames.nextToken().trim();
          String[] parts = hostname.split(":");
          URL url;
          if (parts.length == 1)
              url = new URL("http", parts[0], "/");
          else if (parts.length == 2)
              url = new URL("http", parts[0], Integer.parseInt(parts[1]), "/");
          else
              throw new RuntimeException("Malformed host name " + hostname);
          System.out.print(url);
          webSites.add(url);
          if (hostnames.hasMoreTokens())
              System.out.print(", ");
      }
      ...
      System.out.print("Extra query string    : ");
      extraQueryString = getProperty("extra_query_string");
      System.out.println(extraQueryString+"<br><br>");
      ...
      System.out.print("Up ramp transactions           : ");
      foo = new Integer(getProperty("workload_up_ramp_transactions"));
      upXactions = foo.intValue()/(remoteClients.size()+1);
      System.out.println(upXactions+"<br>");
      System.out.print("Session run transactions       : ");
      foo = new Integer(getProperty("workload_session_run_transactions"));
      sessionXactions = foo.intValue()/(remoteClients.size()+1);
      System.out.println(sessionXactions+"<br>");
      System.out.print("Down ramp transactions         : ");
      foo = new Integer(getProperty("workload_down_ramp_transactions"));
      downXactions = foo.intValue()/(remoteClients.size()+1);
      System.out.println(downXactions+"<br>");
      ...
      if (useVersion.compareTo("PHP") == 0)
        urlGen = new URLGeneratorPHP(webSites, PHPHTMLPath, PHPScriptPath, extraQueryString);
      else if (useVersion.compareTo("EJB") == 0)
        urlGen = new URLGeneratorEJB(webSites, EJBHTMLPath, EJBScriptPath, extraQueryString);
      else if (useVersion.compareTo("Servlets") == 0)
        urlGen = new URLGeneratorServlets(webSites, ServletsHTMLPath, ServletsScriptPath, extraQueryString);
      else
      {
   ...
   */
  public Vector<URL> getWebServers()
  {
    return webSites;
  }
