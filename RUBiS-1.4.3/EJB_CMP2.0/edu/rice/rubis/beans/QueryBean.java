...
public class QueryBean implements SessionBean 
{
  ...
  public Vector getCurrentItemsInCategory(Integer categoryId, int startingRow, int nbOfRows)
  {
    ...
    try 
    {
      ...
      stmt = conn.prepareStatement("SELECT items.id FROM items WHERE items.category=? AND end_date>=NOW() ORDER BY items.end_date ASC LIMIT ?,?");
  ...
  public Vector getCurrentItemsInCategoryAndRegion(Integer categoryId, Integer regionId, int startingRow, int nbOfRows)
  {
    ...
    {
      conn = dataSource.getConnection();
      stmt = conn.prepareStatement("SELECT items.id FROM items,users WHERE items.category=? AND items.seller=users.id AND users.region=? AND end_date>=NOW() ORDER BY items.end_date ASC LIMIT ?,?");
      stmt.setInt(1, categoryId.intValue());
  ...
  /**
   * Get all the buyNow the user bought on in the last 30 days.
   *
   * @param userId user id
   *
   * @return Vector of buyNow primary keys (can be less than maxToCollect)
   */
  public Vector getUserBuyNow(Integer userId)
  {
    Connection        conn = null;
    PreparedStatement stmt = null;
    Vector v = new Vector();
    ...
    {
      conn = dataSource.getConnection();
      stmt = conn.prepareStatement("SELECT buy_now.id FROM buy_now WHERE buy_now.buyer_id=? AND TO_DAYS(NOW()) - TO_DAYS(buy_now.date)<=30");
      stmt.setInt(1, userId.intValue());
      ResultSet rs = stmt.executeQuery();
      ...
      while (rs.next()) 
      {
        BuyNowPK bnPK = new BuyNowPK(new Integer(rs.getInt("buy_now.id")));
        v.addElement((Object)bnPK);
      };
    }
    catch (SQLException e)
    {
   ...
   * @since 1.0
   */
  public Vector getUserCurrentSellings(Integer userId)
  {
    Connection        conn = null;
    PreparedStatement stmt = null;
    Vector v = new Vector();
   ...
   * @since 1.0
   */
  public Vector getUserPastSellings(Integer userId)
  {
    Connection        conn = null;
    PreparedStatement stmt = null;
    Vector v = new Vector();
