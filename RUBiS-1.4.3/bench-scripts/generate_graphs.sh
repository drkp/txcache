...
# $5 is the number of web servers
...
nhttp=$5
 ...
 if [ -d $1 ]; then
  ...
  for ((i = 0; i < $nhttp; i = i + 1)); do
   gunzip $1"web_server$i.gz"
   bench-scripts/format_sar_output.awk $1"web_server$i"
   gzip -9 $1"web_server$i" &
  done
   if [ -f $1"cjdbc_server.gz" ]; then
     gunzip $1"cjdbc_server.gz"
   fi
   if [ -f $1"cjdbc_server" ]; then
     bench-scripts/format_sar_output.awk $1"cjdbc_server"
  ...
  echo -n plot '"'$1db_server0.cpu.idle.dat'"' title '"'Database'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.cpu.idle.dat'"' title '"'Frontend $i'"' with lines >> $tmpFile;
  done
  ...
  echo "set yrange [0:100]" >> $tmpFile;
  echo -n plot '"'$1db_server0.cpu.busy.dat'"' title '"'Database'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.cpu.busy.dat'"' title '"'Frontend $i'"' with lines >> $tmpFile;
  done
   /usr/bin/gnuplot $tmpFile
  ...
  echo 'set ylabel "Processor usage in %"' >> $tmpFile;
  echo "set yrange [0:100]" >> $tmpFile;
  echo -n plot '"'$1db_server0.cpu.user.dat'"' title '"'Database user'"' with lines, '"'$1db_server0.cpu.system.dat'"' title '"'Database kernel'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.cpu.user.dat'"' title '"'Frontend $i user'"' with lines, '"'$1web_server$i.cpu.system.dat'"' title '"'Frontend $i kernel'"' with lines >> $tmpFile;
  done
   /usr/bin/gnuplot $tmpFile
  ...
  echo -n plot '"'$1db_server0.proc.dat'"' title '"'Database'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.proc.dat'"' title '"'Frontend $i'"' with lines >> $tmpFile;
  done
   /usr/bin/gnuplot $tmpFile
  ...
  echo -n plot '"'$1db_server0.ctxsw.dat'"' title '"'Database'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.ctxsw.dat'"' title '"'Frontend $i'"' with lines >> $tmpFile;
  done
   /usr/bin/gnuplot $tmpFile
  ...
  echo -n plot '"'$1db_server0.disk.tps.dat'"' title '"'Database'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.disk.tps.dat'"' title '"'Frontend $i'"' with lines >> $tmpFile;
  done
   /usr/bin/gnuplot $tmpFile
  ...
  echo -n plot '"'$1db_server0.disk.rtps.dat'"' title '"'Database read'"' with lines, '"'$1db_server0.disk.wtps.dat'"' title '"'Database write'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.disk$i.rtps.dat'"' title '"'Frontend $i read'"' with lines, '"'$1web_server$i.disk.wtps.dat'"' title '"'Frontend $i write'"' with lines >> $tmpFile;
  done
   /usr/bin/gnuplot $tmpFile
  ...
  echo -n plot '"'$1db_server0.disk.brdps.dat'"' title '"'Database read'"' with lines, '"'$1db_server0.disk.bwrps.dat'"' title '"'Database write'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.disk.brdps.dat'"' title '"'Frontend $i read'"' with lines, '"'$1web_server$i.disk.bwrps.dat'"' title '"'Frontend $i write'"' with lines >> $tmpFile;
  done
   /usr/bin/gnuplot $tmpFile
  ...
  echo -n plot '"'$1db_server0.mem.kbmemused.dat'"' title '"'Database'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.mem.kbmemused.dat'"' title '"'Frontend $i'"' with lines >> $tmpFile;
  done
   /usr/bin/gnuplot $tmpFile
  ...
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Amount of memory in KB"' >> $tmpFile;
  echo -n plot '"'$1db_server0.mem.kbmemused.dat'"' title '"'Database memory'"' with lines, '"'$1db_server0.mem.kbcached.dat'"' title '"'Database cache'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.mem.kbmemused.dat'"' title '"'Frontend $i memory'"' with lines, '"'$1web_server$i.mem.kbcached.dat'"' title '"'Frontend $i cache'"' with lines >> $tmpFile;
  done
   /usr/bin/gnuplot $tmpFile
  ...
  echo -n plot '"'$1db_server0.net.rxpck.dat'"' title '"'Database received'"' with lines, '"'$1db_server0.net.txpck.dat'"' title '"'Database transmitted'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.net.rxpck.dat'"' title '"'Frontend $i received'"' with lines, '"'$1web_server$i.net.txpck.dat'"' title '"'Frontend $i transmitted'"' with lines >> $tmpFile;
  done
   /usr/bin/gnuplot $tmpFile
  ...
  echo -n plot '"'$1db_server0.net.rxbyt.dat'"' title '"'Database received'"' with lines, '"'$1db_server0.net.txbyt.dat'"' title '"'Database transmitted'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.net.rxbyt.dat'"' title '"'Frontend $i received'"' with lines, '"'$1web_server$i.net.txbyt.dat'"' title '"'Frontend $i transmitted'"' with lines >> $tmpFile;
  done
   /usr/bin/gnuplot $tmpFile
  ...
  echo -n plot '"'$1db_server0.sock.totsck.dat'"' title '"'Database'"' with lines >> $tmpFile;
  for ((i = 0; i < $nhttp; i = i + 1)); do
      echo -n , '"'$1web_server$i.sock.totsck.dat'"' title '"'Frontend $i'"' with lines >> $tmpFile;
  done
   /usr/bin/gnuplot $tmpFile
